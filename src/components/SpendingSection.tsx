  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <h3 className="text-xs font-semibold text-sky-700 uppercase tracking-wider">Actual Spending</h3>
        <span className="text-xs text-sky-900">from transactions</span>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <StatCard
          ...
          valueColor={netPositive ? 'text-cyan-400' : 'text-orange-400'}
        />
      </div>
      ...
      {topMerchants.length > 0 && (
        <div className="rounded-xl border border-teal-900/40 bg-depth-900 overflow-hidden">
          <div className="px-5 py-3.5 border-b border-teal-900/40">
            <h4 className="text-xs font-semibold text-sky-700 uppercase tracking-wider">Top Spending</h4>
          </div>
              ...
              <tbody className="divide-y divide-white/[0.03]">
                {topMerchants.map((m, i) => (
                  <tr key={m.merchant} className="hover:bg-white/[0.02] transition-colors">
                    <td className="px-5 py-3 text-sky-900 text-xs tabular-nums w-8">{i + 1}</td>
                    <td className="px-5 py-3 text-sky-100">{m.merchant}</td>
                    <td className="px-5 py-3 text-right font-medium text-white tabular-nums">
                      {USD.format(m.total)}
                    </td>
                    <td className="px-5 py-3 text-right text-sky-700 text-xs whitespace-nowrap tabular-nums">
