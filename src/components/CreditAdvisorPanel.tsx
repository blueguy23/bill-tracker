...
function AZEOCardRow({ card }: { card: AZEOCard }) {
  ...
  return (
    <div className={`rounded-xl border p-4 space-y-3 ${card.alertActive ? 'border-amber-500/40 bg-amber-500/[0.04]' : 'border-teal-900/40 bg-depth-800/50'}`}>
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="text-sm font-semibold text-white truncate">{card.accountName}</p>
          {card.isAnchor && (
            <span className="text-[10px] font-semibold text-cyan-400 bg-blue-500/[0.12] rounded-full px-2 py-0.5 mt-0.5 inline-block">
              AZEO Anchor
            </span>
          )}
        </div>
        <div className="text-right shrink-0">
          ...
          <p className="text-xs text-sky-700">{USD.format(card.currentBalance)} / {USD.format(card.creditLimit)}</p>
        </div>
      </div>
      ...
      {card.paydownNeeded > 0 && (
        <div className={`rounded-lg px-3 py-2 text-xs space-y-1 ${utilizationBg(card.currentUtilization)}`}>
          ...
          {!card.isAnchor && (
            <p className="text-sky-500">Pay <span className="font-semibold text-white">{USD.format(card.currentBalance)}</span> → report $0 (AZEO target)</p>
          )}
        </div>
      )}
      ...
      {!card.statementClosingDay && (
        <p className="text-xs text-sky-900">
          Set statement closing date in <a href="/settings" className="text-sky-500 underline underline-offset-2">Settings</a> for close alerts
        </p>
      )}
    </div>
  );
...
export function CreditAdvisorPanel({ data }: CreditAdvisorPanelProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {trend.length > 0 && (
        <div className="rounded-xl border border-teal-900/40 bg-depth-900 overflow-hidden">
          <div className="px-5 py-3.5 border-b border-teal-900/40">
            <p className="text-xs font-semibold text-sky-700 uppercase tracking-wider">30-Day Utilization Trend</p>
          </div>
      ...
      {azeo && (
        <div className="rounded-xl border border-teal-900/40 bg-depth-900 overflow-hidden">
          <div className="px-5 py-3.5 border-b border-teal-900/40 flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold text-sky-700 uppercase tracking-wider">AZEO Paydown Plan</p>
            ...
            {azeo.projectedScore !== null && (
              <div className="text-right shrink-0 ml-4">
                <p className="text-xs text-sky-700">If completed</p>
                <p className="text-lg font-bold text-emerald-400">{azeo.projectedScore}<span className="text-xs text-sky-700">/100</span></p>
              </div>
            )}
          </div>
          ...
          <div className="px-5 py-3 border-t border-teal-900/40">
            <p className="text-xs text-sky-900">
              Projected utilization after AZEO: <span className="text-sky-500">{Math.round(azeo.projectedOverallUtilization * 100)}%</span>
              {' · '}Configure statement dates in <a href="/settings" className="text-sky-500 underline underline-offset-2">Settings</a> to receive close alerts
            </p>
          </div>
        </div>
      )}
