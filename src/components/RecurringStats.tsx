...
function StatCard({ label, value, sub, fromColor, dotColor }: StatCardProps) {
  return (
    <div className={`rounded-xl p-5 border border-teal-900/40 bg-gradient-to-br ${fromColor} to-zinc-900`}>
      ...
      {sub && <p className="mt-2 text-xs text-sky-700">{sub}</p>}
    </div>
  );
}
  ...
  return (
    <div className="space-y-6">
      ...
      {orderedIntervals.map((interval) => {
        ...
        return (
          <div key={interval} className="rounded-xl border border-teal-900/40 bg-depth-900 overflow-hidden">
            <div className="px-5 py-3.5 border-b border-teal-900/40 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-sky-700">({group.length} bill{group.length !== 1 ? 's' : ''})</span>
              </div>
              ...
              <table className="min-w-full text-sm">
                <thead>
                  <tr className="border-b border-teal-900/25">
                    <th className="px-5 py-2.5 text-left text-[11px] font-semibold text-sky-700 uppercase tracking-wider">Name</th>
                    <th className="px-5 py-2.5 text-right text-[11px] font-semibold text-sky-700 uppercase tracking-wider">Amount</th>
                    <th className="px-5 py-2.5 text-right text-[11px] font-semibold text-sky-700 uppercase tracking-wider hidden sm:table-cell">Due Day</th>
                    <th className="px-5 py-2.5 text-right text-[11px] font-semibold text-sky-700 uppercase tracking-wider hidden sm:table-cell">AutoPay</th>
                    {interval !== 'monthly' && (
                      <th className="px-5 py-2.5 text-right text-[11px] font-semibold text-sky-700 uppercase tracking-wider hidden sm:table-cell">Mo. Equiv</th>
                    )}
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/[0.03]">
                  {group.map((b) => (
                    <tr key={b._id} className="hover:bg-white/[0.02] transition-colors">
                      <td className="px-5 py-3 text-sky-100">{b.name}</td>
                      <td className="px-5 py-3 text-right font-medium text-white tabular-nums">
                        {USD.format(b.amount)}
                      </td>
                      <td className="px-5 py-3 text-right text-sky-700 hidden sm:table-cell">
                      ...
                      {interval !== 'monthly' && (
                        <td className="px-5 py-3 text-right text-sky-700 text-xs tabular-nums hidden sm:table-cell">
