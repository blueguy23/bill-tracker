  ...
  if (needsSync) {
    return (
      <div className="rounded-xl border border-blue-500/20 bg-blue-500/[0.06] p-5 flex items-start justify-between gap-4">
        ...
        <button onClick={handleDismiss} className="text-zinc-500 hover:text-zinc-300 transition-colors shrink-0" aria-label="Dismiss">
    ...
    <div className="rounded-xl border border-amber-500/20 bg-amber-500/[0.06] p-5 space-y-4">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-0.5">Bill Tracker uses SimpleFIN to securely read your transaction data. No credentials stored.</p>
        </div>
        <button onClick={handleDismiss} className="text-zinc-500 hover:text-zinc-300 transition-colors shrink-0" aria-label="Dismiss">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M18 6 6 18M6 6l12 12" /></svg>
        </button>
      ...
      <ol className="space-y-3">
        <li className="flex items-start gap-3">
          ...
          <p className="text-sm text-zinc-300">
            Visit{' '}
            <a href="https://beta-bridge.simplefin.org/simplefin/create" target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300 underline underline-offset-2">
        ...
        <li className="flex items-start gap-3">
          ...
          <p className="text-sm text-zinc-300">
            Open your <code className="text-xs bg-zinc-800 px-1.5 py-0.5 rounded font-mono">.env</code> file and add:{' '}
            <code className="text-xs bg-zinc-800 px-1.5 py-0.5 rounded font-mono text-emerald-300">SIMPLEFIN_ACCESS_URL=&lt;paste token here&gt;</code>
          </p>
        </li>
        <li className="flex items-start gap-3">
