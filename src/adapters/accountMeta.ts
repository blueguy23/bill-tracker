...
export async function getAccountMeta(db: StrictDB, accountId: string): Promise<AccountMeta> {
  ...
  return { _id: accountId, statementClosingDay: null, targetUtilization: DEFAULT_TARGET_UTIL };
}
...
export async function listAccountMeta(db: StrictDB, accountIds: string[]): Promise<AccountMeta[]> {
  ...
  return accountIds.map((id) => metaMap.get(id) ?? { _id: id, statementClosingDay: null, targetUtilization: DEFAULT_TARGET_UTIL });
}
