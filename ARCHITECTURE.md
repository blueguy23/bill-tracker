...
| **C3** | `isTransfer()` — hardcoded transfer keywords, diverges from canonical `classifyTransfer.ts` | `app/api/v1/summary/route.ts:5–13` | Should use `classifyTransfer` or stored `isTransfer` field | **HIGH** |
